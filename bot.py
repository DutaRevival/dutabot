...
ALL_QUIZ_QUESTIONS = [
    ...
    {"q": "Duree d un match de foot ?", "a": "90", "choices": ["80 min", "90 min", "120 min"]},
    ...
    {"q": "Grammes dans un kilogramme ?", "a": "1000", "choices": ["500", "1000", "1500"]},
    ...
    if user_id in jumble_sessions and jumble_sessions[user_id].get("active"):
        ...
        if remaining > 0:
            await update.message.reply_text(
                ...
                f"Lettres: >>> {s['letters']} <<<\n"
                f"Mots trouves: {len(s['found_words'])}"
            )
            return
...
async def jumble_timer(user_id, chat_id, context):
    ...
    if user_id in jumble_sessions and jumble_sessions[user_id].get("active"):
        ...
        if points > 0:
            ...
            mots_str = ", ".join([f"{w}({len(normalize(w))}pts)" for w in found])
            await context.bot.send_message(
                chat_id=chat_id,
                text=(
                    f"Temps ecoule!\n\n"
                    f"Mots trouves: {mots_str}\n\n"
                    f"Points gagnes: +{points} pts\n"
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text="Temps ecoule! Aucun mot valide. /jumble pour reessayer!"
            )
...
async def quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, c in enumerate(choices):
        keyboard.append([InlineKeyboardButton(c, callback_data=f"qz_{user_id}_{i}")])
    ...
    asyncio.create_task(quiz_timer(user_id, chat_id, question["a"], context))

async def quiz_timer(user_id, chat_id, answer, context):
...
async def quiz_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = session["username"]
    choices_normalized = session["choices_normalized"]
    choices_display = session["choices_display"]
    chosen_norm = choices_normalized[choice_index]
    chosen_display = choices_display[choice_index]
    ...
    if chosen_norm == correct:
        ...
        await query.edit_message_text(
            f"✅ {chosen_display} - Bonne reponse!\n+15 points! Score: {s} pts\n\n/quiz pour continuer!"
        )
    else:
        await query.edit_message_text(
            f"❌ {chosen_display} - Mauvaise reponse!\nLa bonne reponse etait: {correct.upper()}\n\n/quiz pour continuer!"
        )
...
async def foot_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"Recherche en cours...")
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_form_word(text_norm, normalize(letters)):
        await update.message.reply_text(
            f"❌ '{text}' - Impossible avec ces lettres!\nLettres: >>> {letters} <<<"
        )
        return
    ...
    if is_valid_word(text):
        ...
        remaining = int(180 - elapsed)
        await update.message.reply_text(
            f"✅ '{text}' valide! +{pts} pts\n"
            f"Mots: {len(found)} | Points: {session['total_points']} | Temps: {remaining}s\n"
            f"Lettres: >>> {letters} <<<"
        )
    else:
        await update.message.reply_text(
            f"❌ '{text}' - Mot non reconnu!\nLettres: >>> {letters} <<<"
        )
