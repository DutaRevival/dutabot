...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if valid:
        ...
        xp_earned = get_xp_for_word(pts, difficulty)
        found.append(text)
        session["total_points"] += pts
        session["xp_earned"] = session.get("xp_earned", 0) + xp_earned
        await update.message.reply_text(
...
def main():
    ...
    app.add_handler(CommandHandler("confirmer_paiement", confirmer_paiement))
    app.add_handler(CommandHandler("activer_premium", activer_premium))
    app.add_handler(CommandHandler("duel", duel))
    app.add_handler(CommandHandler("tournoi", tournoi))
    app.add_handler(CommandHandler("profile", profile))
    app.add_handler(CommandHandler("niveau", jumble_level))
    ...
    app.add_handler(CallbackQueryHandler(duel_callback, pattern="^(dinv_|dacc_)"))
    app.add_handler(CallbackQueryHandler(duel_quiz_callback, pattern="^dq_"))
    app.add_handler(CallbackQueryHandler(tournoi_callback, pattern="^(tnew_|tjoin_|tlaunch_)"))
    app.add_handler(CallbackQueryHandler(tournoi_quiz_cb, pattern="^tq_"))
