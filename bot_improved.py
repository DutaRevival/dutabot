...
# ======== DYNAMIC JUMBLE ENGINE ========

WORD_BANK = [
    "reaction", "football", "telegram", "planet", "ordinateur", "camera",
    "internet", "champion", "python", "diamond", "argent", "golden",
    "cameroun", "senegal", "argentine", "science", "musique", "victory",
    "creative", "monster", "capital", "library", "festival", "aventure",
    "galaxie", "mystere", "challenge", "competition", "generation",
    "footballer", "premier", "ranking", "economy", "viral", "community"
]
...
def generate_dynamic_jumble():
    """
    Generate a smarter jumble with reduced repetition.
    """
    global RECENT_JUMBLES
    ...
    for _ in range(100):
        ...
        if jumble not in RECENT_JUMBLES:
            ...
            if len(RECENT_JUMBLES) > 50:
                RECENT_JUMBLES.pop(0)

            return jumble

    return ''.join(random.sample("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 10))
...
async def jumble(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    letters = generate_dynamic_jumble()
